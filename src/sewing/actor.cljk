(ns sewing.actor
  "SewingOperationsActor — the ISCO-08 8153 community sewing actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one sewing operation request
  ...
  The unconditional invariant: the SewingOperationsAdvisor can never
  directly commit a record the SewingOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [sewing.advisor :as advisor]
            [sewing.governor :as governor]
            [sewing.store :as store]))

(defn build-graph
  "Build a compiled SewingOperationsActor graph. `store` implements
  `sewing.store/Store`. `advisor` implements
  `sewing.advisor/Advisor` (defaults to `mock-advisor`).
